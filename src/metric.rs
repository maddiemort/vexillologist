...
pub static MESSAGES_RECEIVED: LazyLock<&'static str> = LazyLock::new(|| {
    let key = "vexillologist.messages_received_total";
    metrics::describe_counter!(key, Unit::Count, "Total number of message events received");
    key
});
...
pub static SCORE_REACTIONS_FAILED: LazyLock<&'static str> = LazyLock::new(|| {
    let key = "vexillologist.score_reactions_failed_count";
    metrics::describe_counter!(
        key,
        Unit::Count,
        "Number of failed attempts to add an emoji reaction to a message"
    );
    key
});
